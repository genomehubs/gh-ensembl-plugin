...
#  my $sp_tab      = $elements->{'species_bar'} ? qq(<ul class="tabs"><li class="species long_tab" style="display:block">Melcinx</li></ul>) : '';
  my $sp_tab      = $elements->{'species_bar'} ? qq(<ul class="tabs"><li class="species long_tab" style="display:block">$elements->{'species_bar'}</li></ul>) : '';
## End GenomeHubs Modifications
  my $tabs        = $elements->{'tabs'} ? qq(<div class="tabs_holder print_hide">$sp_tab $elements->{'tabs'}</div>) : '';
